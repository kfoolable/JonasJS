...
const whereAmI = function (lat, lng) {
  fetch(`https://geocode.xyz/${lat},${lng}?geoit=json`)
    ...
    .then((data) => {
      console.log(data);
      const throttle = data.city;

      if (throttle == 'Throttled! See geocode.xyz/pricing')
        throw new Error(
          'Something went wrong, wait 3 seconds for results to show!'
        );
...
// Jonas solution
const whereAmIJ = function (lat, lng) {
  fetch(
    ...
    .then((res) => {
      ...
      return res.json(); // extracts JSON content from the response
    })
    .then((data) => {
      // use the JSON data (parsed as a JavaScript object)
      // console.log(data);
    ...
    .then((data) => {
      console.log(data); // array output
      renderCountry(data[0]);
    })
    .catch((err) => console.error(err.message));
};

whereAmIJ(52.508, 13.381);
// whereAmIJ(19.037, 72.873);
// whereAmIJ(-33.933, 18.474);
